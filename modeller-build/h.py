...
def builder2( sdb, mm ) :
    #print( '--->builder : ', obj )
    # aln = pickle.loads( obj )
    aln = Alignment(env)
    aln.read( mm )
    print( '--->builder : ', aln.keys(), len(aln ) )
    mm.close()
    return
...
def read_and_build_func( sdb, fname ) :
    ...
    while True:
        aln = read_single_sequence( seqfile, format )
        print( '--->reader : ', aln.keys(), len(aln), sys.getsizeof(aln) )
        siz = sys.getsizeof(aln)
        mm = mmap.mmap( -1, siz*100, access=mmap.ACCESS_WRITE )
        aln.write( mm )

        # obj = MyClass( aln[0].code, sequence_string( aln[0] ) )
        # obj = pickle.dumps( aln )
        threads.append( thread_pool.submit( builder2, sdb, mm ) )
...
def read_single_sequence( seqfile, format ) :
    with lock.acquire() :
        try :
            ...
            print( '\n===>reader exception : ', aln[0].code,  ' : ', len(aln[0].residues),  ' : ',  aln[0]._Sequence__get_nseg(), '\n' )
        return aln
